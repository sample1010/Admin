<template>
  ...
  <cl-dialog v-model="pv.visible" title="預覽" width="500px">
...
import { Delete, PictureFilled, ZoomIn } from "@element-plus/icons-vue";
import dayjs from "dayjs";
import { ElMessage } from "element-plus";
import { isArray, isNumber } from "lodash-es";
import { PropType, computed, reactive, ref, watch, watchEffect } from "vue";
import Draggable from "vuedraggable";
import { fileName, fileSize, fileType } from "../utils";
...
watchEffect(() => {
  console.log(compressedImage.value);
});

// 上傳前
async function beforeUpload(file: any, item?: Item) {
  if (file.size / 1024 / 1024 >= limitSize) {
    ElMessage.error(`上傳文件大小不能超過 ${limitSize}MB!`);
    return false;
  }
...
async function blobUrlToFile(blobUrl: any, filename: any) {
  try {
  ...
  if (fileMb < 1) {
    ...
    reader.onload = (event: any) => {
      ...
      img.onload = () => {
        ...
        // Convert canvas to a new Blob with specified image type
        canvas.toBlob(async (blob: any) => {
          const compressedUrl = URL.createObjectURL(blob);
          const compressedImage = await blobUrlToFile(compressedUrl, file.name);

          uploadFile(compressedImage, item);
        }, file.type);
      };
...
async function uploadFile(file: any, item: any) {
  try {
    // 文件名 uuid + filename
